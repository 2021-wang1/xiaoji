<template>
  <div class="main-content">
        ...
        >
          <el-form-item>
            ...
            >
              ...
              >
              <el-button
                v-if="contents.tableBtnIcon != 1"
                :disabled="dataListSelections.length <= 0"
                @click="HandleClickCriticism"
                icon="el-icon-edit"
                type="danger"
                :loading="readLoding"
      ...
      <div class="table-content">
        ...
        >
          ...
          <el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
            <template slot-scope="scope">
              <el-button
                v-if="isAuth('zuoyetijiao', '查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                type="success"
              ...
              >
              <el-button
                v-if="isAuth('zuoyetijiao', '查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                type="success"
                size="mini"
                @click="addOrUpdateHandler(scope.row.id, 'info')"
              ...
              /></el-button>
              <el-button
                v-if="isAuth('zuoyetijiao', '查看') && contents.tableBtnIcon == 0"
                type="success"
                size="mini"
                @click="addOrUpdateHandler(scope.row.id, 'info')"
                >{{ contents.tableBtnFont == 1 ? '详情' : '' }}</el-button
              >
              <el-button
                v-if="isAuth('zuoyetijiao', '点评') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                type="success"
                icon="el-icon-tickets"
                size="mini"
                @click="zuoyedianpingCrossAddOrUpdateHandler(scope.row, 'cross')"
                >{{ contents.tableBtnFont == 1 ? '点评' : '' }}</el-button
              >
              <el-button
                v-if="isAuth('zuoyetijiao', '点评') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                type="success"
                size="mini"
                @click="zuoyedianpingCrossAddOrUpdateHandler(scope.row, 'cross')"
                >{{ contents.tableBtnFont == 1 ? '点评' : '' }}<i class="el-icon-tickets el-icon--right"
              /></el-button>
              <el-button
                v-if="isAuth('zuoyetijiao', '点评') && contents.tableBtnIcon == 0"
                type="success"
                size="mini"
                @click="zuoyedianpingCrossAddOrUpdateHandler(scope.row, 'cross')"
                >{{ contents.tableBtnFont == 1 ? '点评' : '' }}</el-button
              >
              <el-button
                v-if="isAuth('zuoyetijiao', '修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                type="primary"
                icon="el-icon-edit"
                size="mini"
              ...
              >
              <el-button
                v-if="isAuth('zuoyetijiao', '修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                type="primary"
                size="mini"
                @click="addOrUpdateHandler(scope.row.id)"
              ...
              /></el-button>
              <el-button
                v-if="isAuth('zuoyetijiao', '修改') && contents.tableBtnIcon == 0"
                type="primary"
                size="mini"
                @click="addOrUpdateHandler(scope.row.id)"
                >{{ contents.tableBtnFont == 1 ? '修改' : '' }}</el-button
              ...
              <el-button
                v-if="isAuth('zuoyetijiao', '删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                type="danger"
                icon="el-icon-delete"
                size="mini"
              ...
              >
              <el-button
                v-if="isAuth('zuoyetijiao', '删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                type="danger"
                size="mini"
                @click="deleteHandler(scope.row.id)"
              ...
              /></el-button>
              <el-button
                v-if="isAuth('zuoyetijiao', '删除') && contents.tableBtnIcon == 0"
                type="danger"
                size="mini"
                @click="deleteHandler(scope.row.id)"
                >{{ contents.tableBtnFont == 1 ? '删除' : '' }}</el-button
    ...
    <zuoyedianping-cross-add-or-update
      v-if="zuoyedianpingCrossAddOrUpdateFlag"
      :parent="this"
      ref="zuoyedianpingCrossaddOrUpdate"
    ></zuoyedianping-cross-add-or-update>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async HandleClickCriticism() {
      ...
      // 1. 获取选中作业的文件名称
      const fileNames = this.dataListSelections.map((item) => {
        const url = item.tijiaozuoye
      ...
      // 2. 请求ai答题接口，获取返回
      let aiReturns = [] // 调用ai接口批阅后的结果

      const aiReqList = this.dataListSelections.map((item) => {
        // 当没有提交作业文件时返回一个自定义的已完成的网络请求
        if (!item.tijiaozuoye) {
          return Promise.resolve({ data: { score: 0, cause: '未提交作业' } })
        }

        // 使用FormData传递fileName
        const formData = new FormData()
        formData.append('fileName', item.tijiaozuoye.substring(item.tijiaozuoye.lastIndexOf('/') + 1))

        return this.$http({
          url: 'zuoyedianping/getWorkScore',
          method: 'post',
          data: formData,
      ...
      try {
        aiReturns = await Promise.all(aiReqList)
        console.log('aiRes')
        console.log(aiReturns)
      } catch (e) {
        Message.error('一键批阅失败！err=' + e)
        console.log(e)
      }
      // 3. 请求点评接口，点评内容使用ai答题的返回
      ...
      const readsReq = this.dataListSelections.map((item, index) => {
        return this.$http({
          url: 'zuoyedianping/save',
          method: 'post',
          data: {
            id: ids[index],
            zuoyebianhao: item.zuoyebianhao,
            zuoyebiaoti: item.zuoyebiaoti,
            xuehao: item.xuehao,
            xueshengxingming: item.xueshengxingming,
            wanchengneirong: item.wanchengneirong,
            pingyu: '得分：' + aiReturns[index].data.score + ';' + aiReturns[index].data.cause,
            riqi: nowStr,
      ...
      try {
        const res = await Promise.all(readsReq)
        console.log(res)
        Message.success('批阅成功！')
      } catch (e) {
        Message.error('批阅失败 err=' + e)
        console.log(e)
      }
